...
Prerequisites:
1. **Neo4j** up: from repo root `docker compose -f team_b/docker-compose.yml up -d neo4j` (bolt on :7687).
...
3. **Python** with deps: `pip install -r team_b/requirements.txt` and `pip install -r demo/backend/requirements.txt` (uses Team B's existing `sentence-transformers` / `neo4j`).
