...
def iter_split_text(text: str, chunk_size: int = 900, overlap: int = 180) -> Iterator[str]:
    cleaned = re.sub(r"\s+", " ", text).strip()
    if not cleaned:
        return
    if len(cleaned) <= chunk_size:
...
def iter_source_pieces(path: Path, chunk_size: int = 900, overlap: int = 180) -> Iterator[str]:
    suffix = path.suffix.lower()
    if suffix in {".txt", ".md"}:
        text = path.read_text(encoding="utf-8")
        yield from iter_split_text(text, chunk_size=chunk_size, overlap=overlap)
        return
    if suffix == ".pdf":
        reader = PdfReader(path)
        for index, page in enumerate(reader.pages, start=1):
            try:
                ...
                print(f"Warning: failed to extract page {index} from {path.name}: {exc}")
                continue
            if not text:
                continue
            page_text = f"[Page {index}] {text}"
            yield from iter_split_text(page_text, chunk_size=chunk_size, overlap=overlap)
        return
    raise ValueError(f"Unsupported file type: {path}")
...
def iter_source_chunks(data_dir: Path) -> Iterable[tuple[Path, Iterator[str]]]:
    for path in sorted(data_dir.rglob("*")):
        if path.suffix.lower() not in {".txt", ".md", ".pdf"} or not path.is_file():
            continue
        yield path, iter_source_pieces(path)
...
def rank_chunks_streaming(question: str, data_dir: Path, top_k: int) -> list[SearchResult]:
    question_terms = set(tokenize(question))
    ...
    for path in sorted(data_dir.rglob("*")):
        if path.suffix.lower() not in {".txt", ".md", ".pdf"} or not path.is_file():
            continue
        for piece in iter_source_pieces(path):
            chunk = Chunk(source=path.name, text=piece)
            score = score_chunk(question_terms, chunk)
            if len(best) < top_k:
                best.append(SearchResult(score, chunk))
                best.sort(key=lambda item: item.score, reverse=True)
            elif score > best[-1].score:
                best[-1] = SearchResult(score, chunk)
                best.sort(key=lambda item: item.score, reverse=True)
    positives = [item for item in best if item.score > 0]
    return positives if positives else best


def answer_question(question: str, data_dir: Path) -> dict:
    top_results = rank_chunks_streaming(question, data_dir, top_k=3)
    if not top_results:
...
def require_neo4j_settings(args: argparse.Namespace) -> tuple[str, str, str, str, bool]:
    ...
    username = args.neo4j_username or os.getenv("NEO4J_USERNAME") or os.getenv("NEO4J_USER")
...
def ingest_into_neo4j(data_dir: Path, graph: Neo4jBioGraph, reset: bool = False) -> int:
    ...
    for path, pieces in iter_source_chunks(data_dir):
        first_piece = next(pieces, None)
        if first_piece is None:
            continue
        start = perf_counter()

        def row_iter() -> Iterator[dict]:
            for index, piece in enumerate(chain([first_piece], pieces)):
        ...
        chunk_count = graph.ingest_source(path.as_posix(), path.name, row_iter())
        elapsed = perf_counter() - start
        print(f"Ingested {path.name}: {chunk_count} chunks in {elapsed:.1f}s")
        source_count += 1
    return source_count
